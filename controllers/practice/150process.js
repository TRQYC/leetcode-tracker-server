const neet = [{
    ...
    premium: !0,
    freeLink: "https://www.lintcode.com/problem/591/",
    pattern: "Graphs",
...
prevPatternList = undefined
prevPattern = ''
for (let i = 0; i < neet.length; i++) {
    const problem = neet[i]
    if (problem.neetcode150) {
        if (problem.pattern !== prevPattern) {
            if (prevPatternList) {
                neet150.push(prevPatternList)
            }
            prevPatternList = {
                name: problem.pattern, 
                questions: []
            }
            prevPattern = problem.pattern 
        }
        prevPatternList.questions.push(
                {
                    title:problem.problem,
                    titleSlug:problem.link,
                    questionId:Number(problem.code.split("-")[0]),
                }
        )
        cnt += 1 
    }
}
console.log(neet150[0].questions)
console.log(cnt)
console.log(neet.length)